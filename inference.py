...
API_BASE_URL = os.getenv("API_BASE_URL") or os.getenv("OPENAI_BASE_URL") or "https://api-inference.huggingface.co/models"
...
async def main() -> None:
    ...
    try:
        ...
        elif len(sys.argv) > 1 and sys.argv[1] in TASK_NAMES:
        ...
        for task_name in tasks_to_run:
            ...
            if not task_config:
                print(f"[DEBUG] Task {task_name} not found, skipping", file=sys.stderr, flush=True)
                continue
        ...
        if all_scores:
            ...
            print(f"\n[SUMMARY] average_score={avg:.2f} tasks={len(all_scores)}", file=sys.stderr, flush=True)
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        print(f"Top level unhandled exception: {e}", file=sys.stderr, flush=True)
        sys.exit(1)
